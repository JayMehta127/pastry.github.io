...
<?php
$qry = "SELECT image FROM about WHERE id=2 AND status = 1";
$result = $con->query($qry);
if($result->num_rows > 0){
	$row = $result->fetch_assoc();
}  
...
<div class="about-bottom" style="background-image:url('images/back/<?php echo $row['image']; ?>');">
    <div class="container">
      <?php
            $qry = "SELECT pagetitle,pagetext FROM pages WHERE id = 1 AND status = 1";
            $result = $con->query($qry);
...
<div class="container">
  <div class="row">
<?php
$qry = "SELECT image,firstname,lastname,userrole FROM user WHERE status = 1";
$result = $con->query($qry);
if($result->num_rows > 0){
